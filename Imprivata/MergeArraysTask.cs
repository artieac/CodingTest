...
namespace Imprivata
{
    public class MergeArraysTask : LoggableTest
    {
        ...
        public void mergeArray(int[] a, int[] b, int M)
        {
            ...
            this.OutputLogger.WriteMessage("Starting array a: " + string.Join(",", a));
            this.OutputLogger.WriteMessage("Starting string b: " + string.Join(",", b));
            ...
            while (j > -1)
            {
                ...
                while(i > -1)
                {
                    if(j > -1)
                    {
                        ...
                        {
                            b[insertCounter] = a[i];
                            this.OutputLogger.WriteMessage("Placing " + a[i] + " into element " + insertCounter);
                            i--;
                            insertCounter--;
                        }
                        else
                        {
                            b[insertCounter] = b[j];
                            this.OutputLogger.WriteMessage("Placing " + b[j] + " into element " + insertCounter);
                            j--;
                            insertCounter--;
                        }
                    }
            ...
            this.OutputLogger.WriteMessage("Sorted List " + string.Join(",", b));
        }
    }
}
